...
with_test <- TRUE
if(any(duplicated(IDs))){stop("replicates: non-unique IDs found!")}
...
intersection <- new_intersection[new_intersection[,7] != -1,,drop=FALSE]
if(at_new == 1){break}
}
if(with_test){
...
intersection <- intersection[intersection[,7] > 0,]
}
