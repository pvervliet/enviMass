...
if(any(Tasks_to_redo == "TRUE")){
use_summary[
match(names(Tasks_to_redo)[Tasks_to_redo == "TRUE"], use_summary[,1])
,2] <- "WAITING"
}
}
...
mat[use_summary[,2] == "excluded",2] <- "background-color: lightblue"
mat[use_summary[,2] == "WAITING",2] <- "background-color: red"
...
mat[use_summary[,1] == "excluded",1] <- "background-color: lightblue"
mat[use_summary[,1] == "excluded / not run",1] <- "background-color: grey"
mat[use_summary[,1] == "Calculation required",1] <- "background-color: red"
use_summary[,2] <- ""
...
if(any(ls() == "logfile")){stop("\n illegal logfile detected _1 in summary_html.r!")}
