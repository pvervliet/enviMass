...
if(any(ls()=="logfile")){stop(paste("\n illegal logfile detected _1 in workflow_node.r at",name_output))}
if(
...
summa[(logfile$summary[,1]==name_summary),2]<<-"done"
output$summa_html<-renderText(enviMass::UCBRR9756L(summa, logfile$Tasks_to_redo));
...
save(logfile,file=file.path(as.character(logfile[[1]]),"logfile.emp"));
if(any(ls()=="logfile")){stop(paste("\n illegal logfile detected _2 in workflow_node.r at",name_output))}
}
