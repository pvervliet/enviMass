...
function(
    ...
    # rules[3]: remove components which are not part of a homologue series ? ###
    ...
    ##########################################################################
    # (0.3) translate peak IDs -> peak table location, only internally #######
    # ... for enviMass, peakIDs are used - but for this function: not ########
    # thus, for results-return at function bottom: the right IDs have to #####
    # be inserted ############################################################
	...
	# on patterns ##################
    if(length(pattern[[1]])>1){
    	pattern2<-pattern # for returning the correct table
	    ...
	    # Pairs entry not used in combine()
    }
	# on adducts ##################
    if(length(adduct[[1]])>1){
    	adduct2<-adduct # for returning the correct table
	...
	# on homologues ##################
    if(length(homol[[1]])>1){
    	homol2<-homol # for returning the correct table
            ...
            if(pattern[["Patterns"]][intord[i],"group ID"]!="0"){
                ...
                for(k in 1:length(allpeaks)){
                    if(pattern[[1]][allpeaks[k],7]!="0"){
						get3<-as.numeric(strsplit(as.character(pattern[["Patterns"]][allpeaks[k],"to ID"]),"/")[[1]]);	# to ID
						get4<-strsplit(as.character(pattern[["Patterns"]][allpeaks[k],"isotope(s)"]),"/")[[1]]; 			# isotope(s)
						get5<-strsplit(as.character(pattern[["Patterns"]][allpeaks[k],"mass tolerance"]),"/")[[1]];		# mass tolerance
