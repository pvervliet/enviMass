WVEKL7373U <- function(mean_intensities, max_intensities, past_incidents,
current_incidents) {
.gtz <- function(what)
{
return(what[what > 0]);
...
if(length(past_incidents)>0){
cc<-c(hist(log10(past_incidents), breaks=100, plot=FALSE), 0)
cc<-max(cc$counts)
}else{
cc<-(0)
}
if(length(current_incidents)>0){
