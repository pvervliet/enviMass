...
as.numeric(binRTs),
as.numeric(out[order(out[,3],decreasing=FALSE),5]),
as.integer(what)
)
...
as.numeric(binmzs),
as.numeric(out[order(out[,1],decreasing=FALSE),5])
)
maxint1<-max(RTbin[,1]);
maxint2<-max(mzbin[,1]);
labelRT<-seq(RTlimit[1],RTlimit[2],((RTlimit[2]-RTlimit[1])/3));
...
setbar(plotstate)
if(at1$x>=0 & at1$y>2.5 & at1$y<=5){
setplot()
setbar(plotstate)
plotdata(profileList,mzlimit,RTlimit,timelimit,plotstate,farb,plot1=TRUE,plot2=TRUE,plot3=TRUE,plot4=TRUE);
next;
}
if(at1$x>=0 & at1$y>7.5 & at1$y<=8){
