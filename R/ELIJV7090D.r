...
if(length(this)>0){
say<-paste(
"Invalid entries for first concentration warn level found in target table rows number ", paste(as.character(this),collapse=", "),
...
if(length(this)>0){
say<-paste(
"Invalid entries for second concentration warn level found in target table rows number ",paste(as.character(this),collapse=", "),
". Entries must be FALSE or numbers, please revise.",sep=""
)
}
}
...
if(targets_check[i,"ID_internal_standard"]!="FALSE"){
