...
if(length(peaklist[1,])>4){stop("peaklist with > 4 columns not allowed")}
if(!is.numeric(peaklist[,1]) || !is.numeric(peaklist[,2]) || !is.numeric(peaklist[,3]) || !is.numeric(peaklist[,3]) ){stop("peaklist columns not numeric")}
...
mass_def<-c(peaklist[,1]-round(peaklist[,1]))
peaklist2<-peaklist[,-c(2,4)]
uplim<-c(mass_def-(peaklist2[,1]*maxup))
uplim_tol<-abs(max(peaklist4)*maxup*2)
downlim<-c(mass_def-(peaklist2[,1]*maxdown))
downlim_tol<-abs(max(peaklist4)*maxdown*2)
peaklist2<-cbind(peaklist2,uplim,downlim)
peaklist2<-as.matrix(peaklist2)
max_delmz<-c(4*max(peaklist4))
scaled<-c(abs(maxmz-minmz),abs(maxrt+minrt),shift)
if(any(scaled==0)){
scaled[scaled==0]<-1
}
peaklist3<-cbind(peaklist2[,c(1,2)],mass_def)
peaklist3<-as.matrix(peaklist3)
bounds<-matrix(ncol=2,nrow=4,0)
marked<-matrix(ncol=3,nrow=length(peaklist2[,1]),0)
marked[,1]<--1;
marked[,3]<-c(1:length(peaklist2[,1]))
colnames(marked)<-c("sweep","done","ID")
tupels<-matrix(nrow=vec_size,ncol=7,0)
tupeldo<-c(1);
new_found<-rep(0,length(peaklist2[,1]))
new_found_ceiling_UA<-rep(0,length(peaklist2[,1]))
new_found_ceiling_LA<-rep(0,length(peaklist2[,1]))
ceiled_UA<-FALSE
ceiled_LA<-FALSE
new_found_floor_UA<-rep(0,length(peaklist2[,1]))
new_found_floor_LA<-rep(0,length(peaklist2[,1]))
floored_UA<-FALSE
floored_LA<-FALSE
dist_ID<-c()
dist_dist<-c()
...
a<-c(0)
use<-c(1)
along<-rep(0,length(peaklist2[,1]))
along[1]<-1
...
if(use2<1){stop("debug me on issue_4 - use2=-1,<1")}
.Call("node_delete",
use,
peaklist3,
peakTree3,
...
if(sum(peakTree3[,3]!=0)>1){stop("debug me on issue _5: peakTree3 was not fully emptied in NN-search!")}
if(any(peakTree3[,3]>1)){stop("debug me on issue _6: peakTree3 corrupted!")}
}else{
along<-sample(
...
bounds[3,1]<-(peaklist2[use,3]-shift-uplim_tol)
bounds[3,2]<-(peaklist2[use,3]+uplim_tol)
bounds[4,1]<-(peaklist2[use,4]-downlim_tol)
bounds[4,2]<-(peaklist2[use,4]+shift+downlim_tol)
...
if( (mass_def[use]+(maxup*maxmz)+uplim_tol)>=.5 ){
bounds[3,1]<-(peaklist2[use,3]-shift-1-uplim_tol)
bounds[3,2]<-(peaklist2[use,3]-1+uplim_tol)
bounds[4,1]<-(peaklist2[use,4]-1-downlim_tol)
bounds[4,2]<-(peaklist2[use,4]+shift-1+downlim_tol)
.Call("search_kdtree_homol",
peaklist2,
peakTree,
bounds,
...
if((mass_def[use]+(maxdown*maxmz)-downlim_tol)<=-.5){
bounds[3,1]<-(peaklist2[use,3]-shift+1-uplim_tol)
bounds[3,2]<-(peaklist2[use,3]+1+uplim_tol)
bounds[4,1]<-(peaklist2[use,4]+1-downlim_tol)
bounds[4,2]<-(peaklist2[use,4]+shift+1+downlim_tol)
...
bounds[3,1]<-(peaklist2[use,3]-uplim_tol)
bounds[3,2]<-(peaklist2[use,3]+shift+uplim_tol)
bounds[4,1]<-(peaklist2[use,4]-shift-downlim_tol)
bounds[4,2]<-(peaklist2[use,4]+downlim_tol)
.Call("search_kdtree_homol",
peaklist2,
peakTree,
bounds,
...
if((mass_def[use]-(maxup*maxmz)-uplim_tol)<=-.5){
bounds[3,1]<-(peaklist2[use,3]+1-uplim_tol)
bounds[3,2]<-(peaklist2[use,3]+shift+1+uplim_tol)
bounds[4,1]<-(peaklist2[use,4]-shift+1-downlim_tol)
bounds[4,2]<-(peaklist2[use,4]+1+downlim_tol)
.Call("search_kdtree_homol",
peaklist2,
peakTree,
bounds,
...
tupels<-tupels[tupels[,1]!=0,,drop=FALSE]
...
if(any(duplicated(x)) & (HS_length<4)){next}
y<-peaklist3[
merged_tupels[i,1:(HS_length+1)],2
]
if(any(duplicated(y)) & (HS_length<4)){next}
...
if(any(duplicated(HS_IDs))){stop("debug me on issue _6: duplicated IDs in similarity grouping of HS!")}
at<-1;
leng<-5000;
from<-rep(0,leng);
to<-rep(0,leng);
...
if(any(duplicated(x_a)) ){next}
y_a<-peaklist[peaks_a,3]
if(any(duplicated(y_a))){next}
...
if(any(duplicated(x_b)) ){next}
y_b<-peaklist[peaks_b,3]
if(any(duplicated(y_b))){next}
...
SS_res_a[!is.na(match(peaks_a,peaks_b))]<-0
...
SS_res_b[!is.na(match(peaks_b,peaks_a))]<-0
...
if(any(HS_IDs[,4]==0)){stop("debug me on issue _7: ungrouped HS formed.")}
}else{
...
peaklist[as.numeric(homol[["Peaks in homologue series"]][i,"peak ID"]),"peak_ID"]
if(homol[["Peaks in homologue series"]][i,"to ID"]!="0"){
those<-as.numeric(strsplit(homol[["Peaks in homologue series"]][i,"to ID"],"/")[[1]])
homol[["Peaks in homologue series"]][i,"to ID"]<-paste0(as.character(peaklist[those,"peak_ID"]),collapse="/")
}
}
for(i in 1:dim(homol[["Homologue Series"]])[1]){
those<-as.numeric(strsplit(homol[["Homologue Series"]][i,"peak IDs"],",")[[1]])
homol[["Homologue Series"]][i,"peak IDs"]<-paste0(as.character(peaklist[those,"peak_ID"]),collapse=",")
}
return(homol);
}
