...
as.numeric(MSlist[[4]][[2]][MSlist[[5]][k,1]:MSlist[[5]][k,2],1]),
as.numeric(MSlist[[4]][[2]][MSlist[[5]][k,1]:MSlist[[5]][k,2],3]),
as.numeric(MSlist[[4]][[2]][MSlist[[5]][k,1]:MSlist[[5]][k,2],2]),
as.integer(order(MSlist[[4]][[2]][MSlist[[5]][k,1]:MSlist[[5]][k,2],2],decreasing=TRUE)),
as.integer(order(MSlist[[4]][[2]][MSlist[[5]][k,1]:MSlist[[5]][k,2],3],decreasing=FALSE)),
as.numeric(dmzdens),
as.integer(ppm2),
as.numeric(drtdens),
as.integer(merged2),
...
with_test <- FALSE
if(with_test){
if( length(unique(clusters[,10])) != (sum(clusters[,1]!=0) - sum(clusters[,8])) ) stop("Negative test in mzclust_pl _1")
clst <- (unique(MSlist[[4]][[2]][(MSlist[[5]][k,1]):(MSlist[[5]][k,2]),6]))
for(i in clst){
if( anyDuplicated(MSlist[[4]][[2]][(MSlist[[5]][k,1]):(MSlist[[5]][k,2]),"RT"][MSlist[[4]][[2]][(MSlist[[5]][k,1]):(MSlist[[5]][k,2]),6] == i]) != 0 ) stop("Negative test in mzclust_pl _2")
}
}
