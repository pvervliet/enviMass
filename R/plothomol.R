...
if(plotlegend==TRUE){
  ...
  plot(homol[["homol_peaks_relat"]][,3],counts,type="h",col=coloring,lwd=1.5,xlab="Series m/z difference",ylab="Moving window count",xlim=c(min(homol[["homol_peaks_relat"]][,3]),max(homol[["homol_peaks_relat"]][,3])))
  ##########################################################################
