...
get_heads <- match(names(heads_summary), names(heads))
...
if(any(MS1_consec > 0)){
scanTypes <- match(data.frame(t(cbind(heads_short, MS1_consec))), data.frame(t(cbind(heads_summary, method_setup$MS1_consec))))
}
}
}
...
MSlevel = logfile$parameters$peak_MSlevel,
...
use_peak_perc_cut <- 0
estim_values <- try({enviMass::JPDVU3369R(MSlist, plotit = FALSE, n = 2000, m = 5)}, silent = TRUE)
