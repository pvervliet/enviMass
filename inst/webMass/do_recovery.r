...
if(!any(measurements[measurements[,"Mode"] == "positive", "ID"] == to_ID)){
...
if(grepl("!",from_quant)){
if(grepl("no target matches", from_quant)){
from_quant <- "0"
}else{
next;
}
...
if(grepl("!", to_quant)){
if(grepl("no target matches", to_quant)){
to_quant <- "0"
}else{
next;
}
...
from_ID<-those_files[i,"ID"]
to_ID<-those_files[i,"tag2"]
if(!any(measurements[measurements[,"Mode"]=="negative","ID"]==to_ID)){
cat("\n WARNING: Missing relation for spiked file detected! Please revise");
next;
}
