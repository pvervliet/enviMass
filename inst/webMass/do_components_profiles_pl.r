...
if(
	...
	if(logfile$workflow[names(logfile$workflow) == "EIC_correlation"] == "yes"){
		...
		if(length(forIDs) > 0){			
			...
			if(dim(found)[1] > 0){
				...
				###################################################################
				for(j in 1:dim(found)[1]){ # insert links
	...
	if(logfile$workflow[names(logfile$workflow) == "isotopologues"] == "yes"){	
		...
		if(length(forIDs) > 0){
			...
			if(dim(found)[1] > 0){
				...
				for(j in 1:dim(found)[1]){ # insert links
					# insert PROFILE LINKS #######################################
					if(found[j,1] == 0){not_found3 <- (not_found3 + 1); next} # e.g., peak blind-removed 
	...
	if(logfile$workflow[names(logfile$workflow)=="adducts"]=="yes"){
		...
		if(length(forIDs) > 0){
			...
			if(dim(found)[1] > 0){
				...
				###################################################################
				for(j in 1:dim(found)[1]){ # insert links	
					if(found[j,1]==0){not_found4<-(not_found4+1);next} # e.g., peak blind-removed 
	...
	if( # run filering & insert values or ...
		...
		# clean isotopologues ####################################################
		#cut_delRT_isot<-median(anaA$delRT_isot)
