...
file.exists(file.path(logfile[[1]],"results","links_peaks_pos"))
){
...
load(file.path(as.character(logfile[[1]]),"results","links_profiles_pos"), envir = as.environment(".GlobalEnv"));
...
clusterExport(cl = clus, varlist = c("peaks", "logfile"), envir = environment())
cluster_results <- clusterMap(
cl = clus,
fun = enviMass:::DKIJN4505X,
forID = as.list(forIDs),
what = "EICs",
RECYCLE = TRUE,
SIMPLIFY = FALSE,
USE.NAMES = FALSE,
...
clusterEvalQ(cl = clus,{rm(list = ls()); gc(verbose = FALSE); NULL})
found <- do.call(rbind, cluster_results[cluster_results != "failed"])
found <- found[found[,1]!=0,, drop = FALSE]
found <- found[found[,2]!=0,, drop = FALSE]
if(dim(found)[1] > 0){
found <- found[order(found[,1], found[,2]),]
for(j in 1:dim(found)[1]){
inserted1<-(inserted1+1);
prof1<-peaks[found[j,1],"profileIDs"][[1]]
prof2<-peaks[found[j,2],"profileIDs"][[1]]
if(FALSE){
cat("\n");
cat(peaks[found[j,1],"RT"]);cat(" - ")
cat(peaks[found[j,2],"RT"])
}
...
found <- found[order(found[,1], found[,2]),]
for(j in 1:dim(found)[1]){
if(found[j,1] == 0){not_found3 <- (not_found3 + 1); next}
if(found[j,2] == 0){not_found3 <- (not_found3 + 1); next}
...
found <- found[order(found[,1], found[,2]),]
for(j in 1:dim(found)[1]){
if(found[j,1]==0){not_found4<-(not_found4+1);next}
if(found[j,2]==0){not_found4<-(not_found4+1);next}
...
links_profiles_pos[[at_entry]][["group"]] <<- prof_all_IDs[prof_all_IDs != along[i]]
}
}
if(with_bar){close(pBar)}
...
file.exists(file.path(logfile[[1]],"results","links_peaks_neg"))
){
...
load(file.path(as.character(logfile[[1]]),"results","links_profiles_neg"),envir=as.environment(".GlobalEnv"));
measurements <- read.csv(file=file.path(logfile[[1]],"dataframes","measurements"),colClasses = "character");
...
links_profiles_neg[[at_entry]][["group"]] <<- prof_all_IDs[prof_all_IDs != along[i]]
}
}
if(with_bar){close(pBar)}
...
if(any(ls()=="profileList_neg")){stop("\n illegal profileList_neg detected _1 in do_components_profiles_pl.r!")}
if(any(ls()=="links_profiles_neg")){stop("\n illegal links_profiles_neg detected _1 in do_components_profiles_pl.r!")}
if(any(objects(envir=as.environment(".GlobalEnv"))=="profileList_neg")){rm(profileList_neg,envir=as.environment(".GlobalEnv"))}
if(any(objects()=="profileList_neg")){rm(profileList_neg)}
if(any(objects(envir=as.environment(".GlobalEnv"))=="links_peaks_neg")){rm(links_peaks_neg,envir=as.environment(".GlobalEnv"))}
if(any(objects()=="links_peaks_neg")){rm(links_peaks_neg)}
