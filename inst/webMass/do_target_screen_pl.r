...
stop("Differing profileList dimensions_1 - report this issue immediately!")
}
...
mztol<-as.numeric(logfile$parameters$tar_dmz)
ppm<-as.logical(as.character(logfile$parameters$tar_ppm))
cutint<-as.numeric(logfile$parameters$tar_intcut)
int_tol<-as.numeric(logfile$parameters$tar_inttol)
RT_tol_outside<-as.numeric(logfile$parameters$tar_drt1)
RT_tol_inside<-as.numeric(logfile$parameters$tar_drt2)
...
many<-0
many_unamb<-0
res_target_pos_screen<-list()
do_at<-1
if(length(target_pos_screen_listed)>0){
for(i in do_at:length(target_pos_screen_listed)){
if(length(target_pos_screen_listed[[i]])>0){
res_target_pos_screen[[i]]<-list()
for(m in 1:length(target_pos_screen_listed[[i]])){
...
results_screen_target_pos <- enviMass::QGFEB5722K(
...
names(links_peaks_pos[[at_entry]]) <- c("target", "IS", "EIC_correl", "isotop", "adducts", "homol")
profileList_pos[["peaks"]][res_target_pos_screen[[i]][[m]][[k]]$Peaks[a,2],5]<<-at_entry
profileList_pos_copy[["peaks"]][res_target_pos_screen[[i]][[m]][[k]]$Peaks[a,2],5]<<-at_entry
links_peaks_pos[[at_entry]][[1]][[1]] <- list()
links_peaks_pos[[at_entry]][[1]][[1]][[1]] <- names(pattern)[i]
links_peaks_pos[[at_entry]][[1]][[1]][[2]] <- local_score
}else{
at_entry<-profileList_pos[["peaks"]][res_target_pos_screen[[i]][[m]][[k]]$Peaks[a,2],5]
at_list<-(length(links_peaks_pos[[at_entry]][[1]])+1)
links_peaks_pos[[at_entry]][[1]][[at_list]] <- list()
links_peaks_pos[[at_entry]][[1]][[at_list]][[1]] <- names(pattern)[i]
links_peaks_pos[[at_entry]][[1]][[at_list]][[2]] <- local_score
}
}
}
}
...
stop("Differing profileList dimensions_2 - report this issue immediately!")
}
...
target_neg_screen_listed<-list()
set_ID<-seq(1:length(profileList_neg[[4]]))
for(i in 1:length(screen_list)){
if(any(is.na(screen_list[[i]]==FALSE))){
target_neg_screen_listed[[i]]<-list()
for(j in 1:length(screen_list[[i]])){
if(screen_list[[i]][[j]]!="FALSE"){
profs<-as.numeric(strsplit(screen_list[[i]][[j]]," / ")[[1]])
for(k in 1:length(profs)){
...
many<-0
many_unamb<-0
res_target_neg_screen<-list()
if(length(target_neg_screen_listed)>0){
for(i in 1:length(target_neg_screen_listed)){
if(length(target_neg_screen_listed[[i]])>0){
res_target_neg_screen[[i]]<-list()
for(m in 1:length(target_neg_screen_listed[[i]])){
...
targets<-read.table(file=file.path(logfile[[1]],"dataframes","targets.txt"),header=TRUE,sep="\t",colClasses = "character");
results_screen_target_neg<-enviMass::QGFEB5722K(
...
names(links_peaks_neg[[at_entry]]) <- c("target", "IS", "EIC_correl", "isotop", "adducts", "homol")
profileList_neg[[2]][res_target_neg_screen[[i]][[m]][[k]]$Peaks[a,2],5]<<-at_entry
profileList_neg_copy[[2]][res_target_neg_screen[[i]][[m]][[k]]$Peaks[a,2],5]<<-at_entry
links_peaks_neg[[at_entry]][[1]][[1]] <- list()
links_peaks_neg[[at_entry]][[1]][[1]][[1]] <- names(pattern)[i]
links_peaks_neg[[at_entry]][[1]][[1]][[2]] <- local_score
}else{
at_entry<-profileList_neg[[2]][res_target_neg_screen[[i]][[m]][[k]]$Peaks[a,2],5]
at_list<-(length(links_peaks_neg[[at_entry]][[1]])+1)
links_peaks_neg[[at_entry]][[1]][[at_list]] <- list()
links_peaks_neg[[at_entry]][[1]][[at_list]][[1]] <- names(pattern)[i]
links_peaks_neg[[at_entry]][[1]][[at_list]][[2]] <- local_score
}
}
}
}
