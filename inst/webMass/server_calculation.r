...
say<-enviMass::SLGKI8044M(isotopes, adducts, skipcheck = isolate(input$do_project_check), ignorefiles = ignorefiles, write_tables = FALSE);
...
input$Calc
if(input$Calc){
if(!any(objects(envir = as.environment(".GlobalEnv")) == "do_flow")){
do_flow <<- 0
time_start <<- Sys.time()
}
if(do_flow == 0){
...
summa[,2] <<- "..."
output$summa_html <- renderText(enviMass::UCBRR9756L(summa));
}
...
at_script_do <- paste("do_", at_node, "_pl", ".R", sep = "")
if(!file.exists(at_script_do)){
at_script_do <- paste("do_", at_node, ".R", sep = "")
if(!file.exists(at_script_do)){
at_script_do <- FALSE
...
at_script_dont <- paste("dont_", at_node, "_pl", ".R", sep = "")
if(!file.exists(at_script_dont)){
at_script_dont <- paste("dont_", at_node, ".R", sep = "")
if(!file.exists(at_script_dont)){
at_script_dont <- FALSE
