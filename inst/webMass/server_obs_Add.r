...
rm(IS,IS1,IS2);
enviMass::RNJJT9347D(logfile,down="pattern")
...
rm(targets)
if(say == "Project consistent"){
if(isolate(input$ISlist_save_copy)){
at_time <- Sys.time()
...
rm(IS_current)
if(say=="Project consistent"){
if(isolate(input$targetlist_save_copy)){
at_time<-Sys.time()
at_time<-gsub(" ","_", at_time)
at_time<-gsub("-","", at_time)
at_time<-gsub(":","_", at_time)
...
file_guessed <- enviMass:::PQREX9556F(isolate(input$Measadd_path[[1]]), propose = FALSE)
...
newID<-USLJZ0111O(as.numeric(measurements1[,"ID"]))
file.copy(
...
if(isolate(input$Measadd_type)=="calibration"){
use_profiling<-"FALSE"
...
use_profiling <- "TRUE"
start_date<-as.character(isolate(input$Measadd_recov_date))
start_time<-as.character(isolate(input$Measadd_recov_time))
tag1<-"FALSE"
...
measurements3[,"Date"] <- enviMass::KFWHN0433B(measurements3[,"Date"]);
measurements3[,"date_end"] <- enviMass::KFWHN0433B(measurements3[,"date_end"]);
...
if(doit=="TRUE"){
if( isolate(input$Measadd_type)!="calibration" ){
enviMass::RNJJT9347D(logfile,down="peakpicking",single_file=TRUE,except="calibration")
output$summa_html <- renderText(enviMass::UCBRR9756L(logfile$summary, logfile$Tasks_to_redo));
}
if( isolate(input$Measadd_type)=="calibration" ){
enviMass::RNJJT9347D(logfile,down="peakpicking",single_file=TRUE)
enviMass::RNJJT9347D(logfile,down="calibration",single_file=TRUE)
output$summa_html <- renderText(enviMass::UCBRR9756L(logfile$summary, logfile$Tasks_to_redo));
}
}
...
newID<-USLJZ0111O(as.numeric(measurements_1[,"ID"]))
}else{
newID<-1
}
if(
...
if(isolate(input$Modif_type)=="calibration"){
use_profiling<-"FALSE"
...
measurements3[measurements3[,"ID"]==atID,"Date"]<-enviMass::KFWHN0433B(measurements3[measurements3[,"ID"]==atID,"Date"]);
...
measurements3[measurements3[,"ID"]==atID,"date_end"]<-enviMass::KFWHN0433B(measurements3[measurements3[,"ID"]==atID,"date_end"]);
...
measurements3[for_those,"Date"]<-enviMass::KFWHN0433B(measurements3[for_those,"Date"]);
...
measurements3[for_those,"date_end"]<-enviMass::KFWHN0433B(measurements3[for_those,"date_end"]);
...
enviMass::RNJJT9347D(down="quantification",check_node=TRUE,check_TP=c("TRUE"))
enviMass::RNJJT9347D(down="calibration")
output$summa_html <- renderText(enviMass::UCBRR9756L(logfile$summary, logfile$Tasks_to_redo));
enviMass::DODNL4951M(session)
}
measurements <- read.csv(file=file.path(logfile[[1]],"dataframes","measurements"),colClasses = "character")
...
measurements4[,"date_end"]<-enviMass::KFWHN0433B(measurements4[,"date_end"]);
measurements4[,"time_end"]<-as.character(isolate(input$Modif_calgroup_time2))
measurements4[,"LOD"]<-"FALSE"
...
newID<-enviMass::USLJZ0111O(as.numeric(c(measurements3[,"ID"],measurements4[,"ID"]))) # here, measurements4 still partly contain duplicated, old IDs
...
names(cal_models_pos)<<-isolate(input$Copy_cal_group)
...
names(cal_models_neg)<<-isolate(input$Copy_cal_group)
...
enviMass::RNJJT9347D(down="LOD",single_file=TRUE)
enviMass::RNJJT9347D(down="calibration",single_file=TRUE)
output$summa_html <- renderText(enviMass::UCBRR9756L(logfile$summary, logfile$Tasks_to_redo));
enviMass::DODNL4951M(session)
measurements <- read.csv(file=file.path(logfile[[1]],"dataframes","measurements"),colClasses = "character")
measurements_tab <- measurements
...
if(any_include){
enviMass::RNJJT9347D(down="quantification",check_node=TRUE,check_TP=c("TRUE"))	# is this optional? after all, the sets are just removed ...
enviMass::RNJJT9347D(down="calibration")
output$summa_html <- renderText(enviMass::UCBRR9756L(logfile$summary, logfile$Tasks_to_redo));
enviMass::DODNL4951M(session) # stops, in combination with Tasks_to_redo, invalid selections in the calibration tab!
}
measurements <- read.csv(file=file.path(logfile[[1]],"dataframes","measurements"),colClasses = "character")
measurements_tab <- measurements
measurements_tab[,1] <- as.numeric(measurements_tab[,1])
...
file_ending <- enviMass::MYRPB6533U(getfiles[i], check = TRUE)
if(
file.exists(filepath) & file_ending # in case of modifications meanwhile
){
cat(paste("\n   processing file # ",i,sep=""));
file_ending <- enviMass::MYRPB6533U(getfiles[i], check = FALSE)
measurements1<-read.csv(file=file.path(logfile[[1]],"dataframes","measurements"),colClasses = "character");
nameit<-names(measurements1);
measurements1<-measurements1[measurements1[,1]!="-",,drop=FALSE]
if(!isolate(input$Import_file_folder_overwrite)){
...
file_guessed <- enviMass:::PQREX9556F(getfiles[i], propose = TRUE)
if(isolate(input$Import_file_folder_namedate)){
...
at_date <<- enviMass::GEMMM7313P(all_dates,get_min=FALSE)
at_date <<- enviMass::ACTUB0941T(Date=at_date,increment=1);#cat(paste("\n",at_date))
}
}
newID <- as.character(USLJZ0111O(as.numeric(measurements1[,"ID"])))
if(file_ending == ".mzXML"){
file.copy(
from=filepath,
to=file.path(logfile[[1]],"files",paste(newID,".mzXML",sep="")),
overwrite=TRUE)
if( file.exists(file.path(logfile[[1]],"files",paste(newID,".mzXML",sep=""))) ){
...
if(many > 0){
enviMass::RNJJT9347D(down = "peakpicking", single_file = TRUE)
logfile$summary[1,2] <<- "TRUE"
output$summa_html <<- renderText(enviMass::UCBRR9756L(logfile$summary, logfile$Tasks_to_redo));
save(logfile, file = file.path(as.character(logfile[[1]]), "logfile.emp"));
...
enviMass::RNJJT9347D(down="peakpicking",single_file=FALSE)
output$summa_html <- renderText(enviMass::UCBRR9756L(logfile$summary, logfile$Tasks_to_redo));
enviMass::DODNL4951M(session)
cat(" done. \n")
}else{
...
enviMass::CQJFW8426U(logfile,ranges_overview)
})
})
